...
<xsl:param name="term1">0</xsl:param>
...
<xsl:variable name="term_string"><xsl:if test="$term != 0">&amp;term1=<xsl:value-of select="$term"/></xsl:if><xsl:if test="$term2 != 0">&amp;term2=<xsl:value-of select="$term2"/></xsl:if><xsl:if test="$term3 != 0">&amp;term3=<xsl:value-of select="$term3"/></xsl:if></xsl:variable>
